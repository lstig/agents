...
| `.claude-plugin/mcp.json` | Canonical `joplin` MCP config, referenced explicitly by the `workflow-skills` plugin. |
...
- Adding a skill means updating three places: the skill directory, the README table, and (if it should install via Claude Code) a `skills` path in the right marketplace entry.
...
- **Never add a root `.mcp.json`.**
  It's a default plugin MCP location, so every plugin sourced at `./` auto-discovers it — this once leaked the joplin server into `development-skills`.
...
- After marketplace changes, smoke test in a scratch project: add the marketplace from the local path, install both plugins, then check `claude mcp list` (the `joplin` server must appear under `plugin:workflow-skills:` only) and that skills resolve.
